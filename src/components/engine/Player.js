...
import { NathanModel } from '../shared/GameElements';
  ...
  useFrame((state, delta) => {
    ...
    if (moveDirection.length() > 0) {
      ...
      const canMoveTo = (dir) => {
        ...
        const intersects = raycaster.intersectObjects(levelObjects, true);
        
        // FILTER: Dummies für Wand-Kollision ignorieren
        const wallIntersects = intersects.filter(hit => 
          !hit.object.userData.noCollision && 
          !hit.object.parent?.userData.noCollision
        );
    ...
    const allGroundIntersects = raycaster.intersectObjects(scene.children, true);
    
    // FILTER: Dummies auch für die Boden-Prüfung ignorieren!
    const groundIntersects = allGroundIntersects.filter(hit => 
      !hit.object.userData.noCollision && 
      !hit.object.parent?.userData.noCollision
    );
